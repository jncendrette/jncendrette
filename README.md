# 👋 Olá, eu sou José Cendrette

🚀 AI SaaS Builder | 💰 Tech Soluções em Finanças & Tributação
🤖 Construindo soluções para automação contábil, fiscal e adaptação à Reforma Tributária (IVA)

---

## 🎯 Missão

Ajudar empresas e escritórios contábeis a **transitarem com segurança do modelo tributário atual para o novo sistema de IVA**, utilizando:

* Inteligência Artificial
* Automação de processos
* Sistemas SaaS escaláveis

---

## ⚖️ Problema que resolvo

A Reforma Tributária no Brasil cria um cenário de:

* Alta complexidade na transição
* Risco de erros fiscais
* Falta de ferramentas adaptadas ao novo modelo

💡 Meu foco é construir soluções que simplifiquem essa mudança.

---

## 🧠 Tech Stack

![Frontend-Next.js](https://img.shields.io/badge/Frontend-Next.js-000000?style=for-the-badge\&logo=nextdotjs\&logoColor=white)
![Infra-Docker](https://img.shields.io/badge/Infra-Docker-2496ED?style=for-the-badge\&logo=docker\&logoColor=white)
![AI-Claude Code](https://img.shields.io/badge/AI-Claude%20Code-000000?style=for-the-badge)
![AI-GitHub Copilot](https://img.shields.io/badge/AI-GitHub%20Copilot-000000?style=for-the-badge\&logo=github)

![AI](https://img.shields.io/badge/Focus-Artificial%20Intelligence-blue)
![Finance](https://img.shields.io/badge/Expertise-Finance-green)
![SaaS](https://img.shields.io/badge/Building-SaaS-orange)
![Automation](https://img.shields.io/badge/Automation-Business-purple)

---

## 🚀 Projetos em Construção

### 🤖 IVA Transition Engine

Sistema inteligente para simular e adaptar empresas ao novo modelo de IVA

✔ Simulação de carga tributária
✔ Comparação: modelo atual vs novo
✔ Insights automáticos com IA

---

### 📊 AI Fiscal Automation

Automação de rotinas fiscais com inteligência artificial

✔ Classificação automática de operações
✔ Validação fiscal
✔ Redução de erros manuais

---

### 🧾 Smart Tax Assistant

Assistente inteligente para suporte contábil e tributário

✔ Respostas baseadas na legislação
✔ Apoio a escritórios contábeis
✔ Interpretação de regras fiscais

---

## 📈 Roadmap

* [ ] MVP de simulação tributária (IVA)
* [ ] Motor de regras fiscais automatizado
* [ ] Integração com ERPs
* [ ] Dashboard de impacto tributário
* [ ] API para escritórios contábeis

---

## 🌎 Oportunidade

A Reforma Tributária no Brasil abre um mercado bilionário para soluções tecnológicas.

Estou construindo ferramentas para ser parte ativa dessa transformação.

---

## 🤝 Vamos construir juntos

Se você atua com:

* Contabilidade
* Tecnologia
* Consultoria tributária

Ou quer desenvolver soluções nesse mercado:

👉 Vamos conectar.

---

## ✝️ Propósito

Tecnologia com propósito: construir soluções que gerem valor real, organização e impacto — guiado por princípios cristãos.

---
